...
describe( "Selection delete action", () => {
    ...
    it( "should call the selection delete action with the appropriate values", () => {
        const commitSpy = vi.spyOn( store, "commit" );

        deleteSelection( store );

        expect( commitSpy ).toHaveBeenCalledWith( "deleteSelection", {
            song, activePattern: activePatternIndex
        });
    });
    ...
    it( "should clear the current selection", () => {
        ...
        expect( commitSpy ).toHaveBeenCalledWith( "clearSelection" );
    });
    ...
    describe( "on undo", () => {
        it( "should restore the original pattern appropriately", () => {
            ...
            const { undo } = deleteSelection( store );
